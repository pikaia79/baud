...
import (
	...
	"github.com/tiglabs/baudengine/mygate/mysql/netutil"
	"github.com/tiglabs/baudengine/mygate/mysql/sqltypes"
	"github.com/tiglabs/baudengine/mygate/mysql/stats"
	"github.com/tiglabs/baudengine/mygate/mysql/tb"
...
func (l *Listener) handle(conn net.Conn, connectionID uint32, acceptTime time.Time) {
	...
	case authServerMethod == MysqlNativePassword && authMethod == MysqlNativePassword:
		...
		if err != nil {
			log.Warn("Error authenticating user using MySQL native password: %v", err)
			c.writeErrorPacketFromError(err)
			return
		}
		c.User = user
	...
	for {
		...
		default:
			log.Error("Got unhandled packet from %s, returning error: %v", c, data)
			c.recycleReadPacket()
