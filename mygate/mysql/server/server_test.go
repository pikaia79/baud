...
import (
	...
	bauderrors "github.com/tiglabs/baudengine/mygate/mysql/errors"
	"github.com/tiglabs/baudengine/mygate/mysql/sqltypes"
	"github.com/tiglabs/baudengine/mygate/mysql/tls"
	"github.com/tiglabs/baudengine/mygate/mysql/tlstest"
...
func TestTLSServer(t *testing.T) {
	...
	serverConfig, err := tls.ServerConfig(
...
func TestErrorCodes(t *testing.T) {
	...
	tests := []struct {
		...
		{
			err: bauderrors.Errorf(
				codepb.Code_INVALID_ARGUMENT,
				"invalid argument"),
			code:     ERUnknownError,
		...
		{
			err: bauderrors.Errorf(
				codepb.Code_RESOURCE_EXHAUSTED,
				"query pool timeout"),
		...
		{
			err:      errors.Wrap(NewSQLError(ERVitessMaxRowsExceeded, SSUnknownSQLState, "Row count exceeded 10000"), "wrapped"),
...
func getMysqlRoot() (string, error) {
	// if the environment variable is set, use that
	if root := os.Getenv("MYSQL_ROOT"); root != "" {
		return root, nil
	} else {
		return "", errors.New("MYSQL_ROOT is not set")
	}
}
...
func runMysql(t *testing.T, params *ConnParams, command string) (string, bool) {
	dir, err := getMysqlRoot()
	if err != nil {
		t.Fatalf("MysqlRoot failed: %v", err)
	}
