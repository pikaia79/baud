...
import (
	...
	rpcpb "github.com/tiglabs/baudengine/proto/mysqlpb"
)
...
func NewSQLErrorFromError(err error) error {
	...
	if len(match) < 2 {
		...
		num := ERUnknownError
		switch code {
		case rpcpb.Code_CANCELED:
			num = ERQueryInterrupted
		case rpcpb.Code_UNKNOWN:
			num = ERUnknownError
		case rpcpb.Code_INVALID_ARGUMENT:
			...
			num = ERUnknownError
		case rpcpb.Code_DEADLINE_EXCEEDED:
			num = ERQueryInterrupted
		case rpcpb.Code_NOT_FOUND:
			num = ERUnknownError
		case rpcpb.Code_ALREADY_EXISTS:
			num = ERUnknownError
		case rpcpb.Code_PERMISSION_DENIED:
			num = ERAccessDeniedError
		case rpcpb.Code_UNAUTHENTICATED:
			num = ERAccessDeniedError
		case rpcpb.Code_RESOURCE_EXHAUSTED:
			num = ERTooManyUserConnections
		case rpcpb.Code_FAILED_PRECONDITION:
			num = ERUnknownError
		case rpcpb.Code_ABORTED:
			num = ERQueryInterrupted
		case rpcpb.Code_OUT_OF_RANGE:
			num = ERUnknownError
		case rpcpb.Code_UNIMPLEMENTED:
			num = ERNotSupportedYet
		case rpcpb.Code_INTERNAL:
			num = ERUnknownError
		case rpcpb.Code_UNAVAILABLE:
			num = ERUnknownError
		case rpcpb.Code_DATA_LOSS:
			num = ERUnknownError
		}
