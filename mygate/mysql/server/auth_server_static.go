...
func InitAuthServerStatic() {
	...
	if *mysqlAuthServerStaticFile != "" && *mysqlAuthServerStaticString != "" {
		...
		log.Fatal("Both mysql_auth_server_static_file and mysql_auth_server_static_string specified, can only use one.")
	}
...
func (a *AuthServerStatic) loadConfigFromParams(file, str string) {
	...
	if file != "" {
		...
		if err != nil {
			log.Fatal("Failed to read mysql_auth_server_static_file file: %v", err)
		}
		jsonConfig = data
	}
	...
	if err := parseConfig(jsonConfig, &a.Entries); err != nil {
		log.Fatal("Error parsing auth server config: %v", err)
	}
}
...
func (sud *StaticUserData) Get() *querypb.CallerID {
	return &querypb.CallerID{Username: sud.value}
}
