...
func Parse(sql string) (Statement, error) {
	...
	if yyParse(tokenizer) != 0 {
		if tokenizer.partialDDL != nil {
			log.Warn("ignoring error parsing DDL '%s': %v", sql, tokenizer.LastError)
			tokenizer.ParseTree = tokenizer.partialDDL
			return tokenizer.ParseTree, nil
		}
		return nil, errors.New(codepb.Code_INVALID_ARGUMENT, tokenizer.LastError.Error())
	}
	return tokenizer.ParseTree, nil
}
...
func ExprFromValue(value sqltypes.Value) (Expr, error) {
	...
	default:
		...
		return nil, errors.Errorf(codepb.Code_INTERNAL, "cannot convert value %v to AST", value)
	}
}
