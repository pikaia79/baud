...
func NewPlanValue(node Expr) (sqltypes.PlanValue, error) {
	...
	case *SQLVal:
		...
		case IntVal:
			...
			if err != nil {
				return sqltypes.PlanValue{}, errors.Errorf(codepb.Code_INVALID_ARGUMENT, "%v", err)
			}
	...
	case ValTuple:
		...
		for _, val := range node {
			...
			if innerpv.ListKey != "" || innerpv.Values != nil {
				return sqltypes.PlanValue{}, errors.New(codepb.Code_UNIMPLEMENTED, "unsupported: nested lists")
			}
	...
	return sqltypes.PlanValue{}, errors.Errorf(codepb.Code_INVALID_ARGUMENT, "expression is too complex '%v'", String(node))
}
