...
func (c *CountersWithSingleLabel) Add(name string, value int64) {
	if value < 0 {
		log.Warn("Adding a negative value to a counter, %v should be a gauge instead", c)
	}
	a := c.getValueAddr(name)
	atomic.AddInt64(a, value)
}
...
func (mc *CountersWithMultiLabels) Add(names []string, value int64) {
	...
	if value < 0 {
		log.Warn("Adding a negative value to a counter, %v should be a gauge instead", mc)
	}
