...
func RegisterPushBackend(name string, backend PushBackend) {
	...
	if _, ok := pushBackends[name]; ok {
		log.Fatal("PushBackend %s already exists; can't register the same name multiple times", name)
	}
...
func emitToBackend(emitPeriod *time.Duration) {
	...
	for range ticker.C {
		...
		if !ok {
			log.Error("No PushBackend registered with name %s", *statsBackend)
			return
		}
		err := backend.PushAll()
		if err != nil {
			// TODO(aaijazi): This might cause log spam...
			log.Warn("Pushing stats to backend %v failed: %v", *statsBackend, err)
		}
	}
}
