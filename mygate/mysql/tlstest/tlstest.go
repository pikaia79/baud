...
func openssl(argv ...string) {
	...
	if err != nil {
		log.Fatal("openssl %v failed: %v", argv, err)
	}
	if len(output) > 0 {
		log.Info("openssl %v returned:\n%v", argv, string(output))
	}
}
...
func CreateCA(root string) {
	log.Info("Creating test root CA in %v", root)
	...
	if err := ioutil.WriteFile(config, []byte(caConfig), os.ModePerm); err != nil {
		log.Fatal("cannot write file %v: %v", config, err)
	}
...
func CreateSignedCert(root, parent, serial, name, commonName string) {
	log.Info("Creating signed cert and key %v", commonName)
