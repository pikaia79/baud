...
In short, " Object: id --> (key --> value*)* "

In detail, 

Field, Object, Space, DB

Each object has an internal 'Unique ID' (UID) generated by the system, which is unique aross the entire system. 
...
Each space has a field as the partition key, and the objects with the same partition key are co-located.
