/*
Package telnet provides TELNET and TELNETS server implementations
...
The TELNET protocol is best known for providing a means of connecting to a remote computer, using a (text-based) shell interface,
and being able to interact with it, (more or less) as if you were sitting at that computer.
...
(The word "video" in that era of computing did not refer to things such as movies. But instead was meant to contrast it with paper. In particular,
the teletype machines, which were typewriter like devices that had a keyboard, but instead of having a monitor had paper that was printed onto.)
...
What individuals would have at their desks, instead of their own computer, would be some type of video terminal.
