...
//
func Match(str, pattern []byte) bool {
	if string(pattern) == "*" {
		return true
	}
	return deepMatch(str, pattern)
}
func deepMatch(str, pattern []byte) bool {
	for len(pattern) > 0 {
		if pattern[0] > 0x7f {
...
func deepMatchRune(str, pattern []byte) bool {
	var sr, pr rune
	var srsz, prsz int
	...
	if len(pattern) > 0 {
		if pattern[0] > 0x7f {
			pr, prsz = utf8.DecodeRune(pattern)
		} else {
