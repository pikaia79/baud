...
type TimeWheel struct {
	...
	slotNum           int              // 槽数量
	timeWheelCB       TimeWheelCB      // 定时器回调函数
...
// NewTimeWheel 创建时间轮
func NewTimeWheel(interval time.Duration, slotNum int, timeWheelCB TimeWheelCB) *TimeWheel {
	if interval <= 0 || slotNum <= 0 || timeWheelCB == nil {
		return nil
	}
	tw := &TimeWheel{
		interval:          interval,
